...
class Operations {
    ...
    getRequireHints(pars) {
        return this.a.patching?.getRequireHints(pars.node.loc.module?.toString(), (0, util_1.locationToString)(pars.node.loc, false, true));
    }
    callFunction(path) {
        ...
        this.solver.addForAllTokensConstraint(calleeVar, listeners_1.TokenListener.CALL_CALLEE, path.node, (t) => handleCall(undefined, t));
        if (p.isMemberExpression() || p.isOptionalMemberExpression()) {
            ...
            this.solver.addForAllTokensConstraint(baseVar, listeners_1.TokenListener.CALL_BASE, path.node, (t) => {
                let callees;
                if (prop !== undefined) {
                ...
                if (callees) {
                    ...
                    this.solver.addForAllTokensConstraint(callees, listeners_1.TokenListener.CALL_CALLEE, { n: path.node, t }, (ft) => handleCall(t, ft));
                }
                if (t instanceof tokens_1.AccessPathToken && (prop === "call" || prop === "apply"))
                    this.solver.addAccessPath(new accesspaths_1.CallResultAccessPath(baseVar), resultVar, t.ap);
            });
        }
    ...
    callFunctionBound(base, t, calleeVar, argVars, resultVar, strings, path) {
        ...
        if (t instanceof tokens_1.FunctionToken)
            ...
            for (let i = 0; i < argVars.length; i++) {
                ...
                if (argVar) {
                    this.solver.addForAllTokensConstraint(argVar, listeners_1.TokenListener.CALL_EXTERNAL, args[i], (at) => this.invokeExternalCallback(at, pars.node, caller));
    ...
    readProperty(base, prop, dst, node, enclosing, extrakey = "") {
        ...
        if (prop !== undefined) {
            ...
            if (dst)
                this.solver.addForAllTokensConstraint(base, listeners_1.TokenListener.READ_BASE_DYNAMIC, lopts, (t) => {
                    if (t instanceof tokens_1.ArrayToken) {
                        this.solver.addSubsetConstraint(this.solver.varProducer.arrayAllVar(t), dst);
                    }
                    else if (!(t instanceof tokens_1.AccessPathToken)) {
                        if (logger_1.default.isInfoEnabled() || options_1.options.approx || options_1.options.approxLoad)
    ...
    readPropertyFromChain(base, prop) {
        ...
        const basePropSpecial = (base instanceof tokens_1.FunctionToken && ecmascript_1.STANDARD_METHODS.get("Function").has(prop)) ||
            (base instanceof tokens_1.AllocationSiteToken && base.kind !== "Object" && ecmascript_1.STANDARD_METHODS.get(base.kind)?.has(prop));
        this.solver.addForAllAncestorsConstraint(base, listeners_1.TokenListener.READ_ANCESTORS, { s: prop }, (t2) => {
            if (basePropSpecial && t2 === this.globalSpecialNatives.get(ecmascript_1.OBJECT_PROTOTYPE))
                return;
    ...
    readPropertyBound(t, prop, dst, dstkey, thist = t) {
        ...
        if (!(t instanceof tokens_1.NativeObjectToken && !t.moduleInfo) && prop !== "prototype") {
    ...
    writeProperty(src, lVar, base, prop, node, enclosing, escapeNode = node, ac = "normal", invokeSetters = true) {
        ...
        if ((0, constraintvars_1.isObjectPropertyVarObj)(base)) {
            ...
            if (invokeSetters)
                if (!(base instanceof tokens_1.NativeObjectToken && !base.moduleInfo) && prop !== "prototype") {
                    this.solver.addForAllAncestorsConstraint(base, listeners_1.TokenListener.WRITE_ANCESTORS, { n: node, s: prop }, (anc) => {
                        if ((0, constraintvars_1.isObjectPropertyVarObj)(anc)) {
                            ...
                            this.solver.addForAllTokensConstraint(setter, listeners_1.TokenListener.WRITE_SETTER, { n: node, s: prop }, writeToSetter);
                            this.solver.addForAllTokensConstraint(setter, listeners_1.TokenListener.WRITE_SETTER_THIS, { t: base }, bindSetterThis);
                        }
                    });
                }
    ...
    requireModule(str, resultVar, path) {
        ...
        if (module_1.default.isBuiltin(str)) {
            ...
            try {
                const filepath = (0, files_1.requireResolve)(str, this.file, f.a, path.node, f);
                if (filepath) {
        ...
        if ((0, types_1.isIdentifier)(dst)) {
            ...
            const assignRequireExtensions = (t) => {
                if (t instanceof tokens_1.NativeObjectToken && t.name === "require.extensions")
                    this.solver.addForAllTokensConstraint(src, listeners_1.TokenListener.WRITE_REQUIRE_EXTENSIONS, path.node, (ft) => this.invokeExternalCallback(ft, path.node, enclosing));
            };
            if (prop !== undefined) {
                this.solver.addForAllTokensConstraint(lVar, listeners_1.TokenListener.WRITE_BASE, dst, (t) => {
                ...
                if (src)
                    this.solver.addForAllTokensConstraint(lVar, listeners_1.TokenListener.WRITE_BASE_DYNAMIC, dst, (t) => {
                        if (t instanceof tokens_1.ArrayToken) {
            ...
            for (const p of dst.properties)
                if ((0, types_1.isRestElement)(p)) {
                    ...
                    this.solver.addForAllTokensConstraint(src, listeners_1.TokenListener.WRITE_OBJECT_PATTERN_REST, p, (t2) => {
                        if (t2 instanceof tokens_1.AllocationSiteToken || t2 instanceof tokens_1.FunctionToken || t2 instanceof tokens_1.NativeObjectToken || t2 instanceof tokens_1.PackageObjectToken) {
                            this.solver.addForAllObjectPropertiesConstraint(t2, listeners_1.TokenListener.WRITE_OBJECT_PATTERN_REST_PROPERTIES, p, (prop) => {
                                if (!matched.has(prop))
            ...
            for (const [i, p] of dst.elements.entries())
                if (p)
                    if ((0, types_1.isRestElement)(p)) {
                        ...
                        this.solver.addForAllTokensConstraint(src, listeners_1.TokenListener.WRITE_ARRAY_PATTERN_REST, p, (t2) => {
                            if (t2 instanceof tokens_1.ArrayToken) {
                                this.solver.addForAllArrayEntriesConstraint(t2, listeners_1.TokenListener.WRITE_ARRAY_PATTERN_REST_ARRAY, p, (prop) => {
    ...
    newObjectToken(n) {
        if (options_1.options.alloc) {
            ...
            if (!options_1.options.widening || !this.solver.fragmentState.widened.has(t)) {
    ...
    newRegExpToken() {
        return this.a.canonicalizeToken(new tokens_1.PackageObjectToken(this.packageInfo, "RegExp"));
    }
    newPromiseToken(n) {
        return this.a.canonicalizeToken(new tokens_1.AllocationSiteToken("Promise", n));
    }
