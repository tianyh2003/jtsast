...
class FragmentState {
    ...
    registerCall(n, enclosing, calleeVar, { native, external, accessor } = {}) {
        if (accessor && !options_1.options.callgraphImplicit)
            return;
        ...
        if (calleeVar)
            (0, util_1.mapGetSet)(this.callToCalleeVars, n).add(calleeVar);
    }
    ...
    *getAllVarsAndTokens() {
    ...
    static emptySizeAndHas = [0, (_t) => false];
    getSizeAndHas(v) {
        if (v) {
            const ts = this.tokens.get(v);
            if (ts) {
