...
class Solver {
    ...
    printDiagnostics() {
        if (options_1.options.printProgress && options_1.options.tty && logger_1.isTTY && !options_1.options.logfile && logger_1.default.level === "info") {
            ...
            if (d > this.diagnostics.lastPrintDiagnosticsTime + 100) {
                ...
                const a = this.globalState;
                const f = this.fragmentState;
                (0, logger_1.writeStdOut)(`Packages: ${a.packageInfos.size}, modules: ${a.moduleInfos.size}, call edges: ${f.numberOfCallToFunctionEdges}, ` +
    ...
    addForAllAncestorsConstraint(t, key, opts, listener) {
        ...
        this.callTokenListener(id, listener, t, true);
        const g = this.globalState.globalSpecialNatives;
        if (g) {
            if (t instanceof tokens_1.ObjectToken)
                this.callTokenListener(id, listener, g.get(ecmascript_1.OBJECT_PROTOTYPE));
            else if (t instanceof tokens_1.ArrayToken) {
                this.callTokenListener(id, listener, g.get(ecmascript_1.ARRAY_PROTOTYPE));
                this.callTokenListener(id, listener, g.get(ecmascript_1.OBJECT_PROTOTYPE));
            }
            else if (t instanceof tokens_1.FunctionToken || t instanceof tokens_1.PrototypeToken || t instanceof tokens_1.ClassToken) {
                this.callTokenListener(id, listener, g.get(ecmascript_1.FUNCTION_PROTOTYPE));
                this.callTokenListener(id, listener, g.get(ecmascript_1.OBJECT_PROTOTYPE));
            }
            else if (t instanceof tokens_1.AllocationSiteToken) {
                if (t.kind === "Promise")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.PROMISE_PROTOTYPE));
                else if (t.kind === "Date")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.DATE_PROTOTYPE));
                else if (t.kind === "RegExp")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.REGEXP_PROTOTYPE));
                else if (t.kind === "Error")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.ERROR_PROTOTYPE));
                else if (t.kind === "Map")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.MAP_PROTOTYPE));
                else if (t.kind === "Set")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.SET_PROTOTYPE));
                else if (t.kind === "WeakMap")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.WEAKMAP_PROTOTYPE));
                else if (t.kind === "WeakSet")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.WEAKSET_PROTOTYPE));
                else if (t.kind === "WeakRef")
                    this.callTokenListener(id, listener, g.get(ecmascript_1.WEAKREF_PROTOTYPE));
