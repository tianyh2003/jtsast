...
function visit(ast, op) {
                ...
                switch (path.node.kind) {
                    ...
                    case "set":
                        ...
                        if (key) {
                            ...
                            if (!options_1.options.oldobj && (options_1.options.approx || options_1.options.approxLoad))
                ...
                for (const p of path.node.properties)
                    if ((0, types_1.isSpreadElement)(p)) {
                        if (options_1.options.objSpread) {
                            ...
                            solver.addForAllTokensConstraint(argVar, listeners_1.TokenListener.OBJECT_SPREAD, p, (t) => {
                                if ((0, constraintvars_1.isObjectPropertyVarObj)(t)) {
                                    solver.addForAllObjectPropertiesConstraint(t, listeners_1.TokenListener.OBJECT_SPREAD, path.node, (prop) => {
    ...
    function visitMemberExpression(path) {
        const dstVar = (0, asthelpers_1.isParentExpressionStatement)(path) ? undefined : vp.nodeVar(path.node);
        ...
        if ((0, asthelpers_1.isCalleeExpression)(path))
            return;
        op.readProperty(op.expVar(path.node.object, path), (0, asthelpers_1.getProperty)(path.node), dstVar, path.node, a.getEnclosingFunctionOrModule(path, op.moduleInfo));
    }
}
exports.visit = visit;
