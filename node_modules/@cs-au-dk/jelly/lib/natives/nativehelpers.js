...
function newSpecialObject(kind, p) {
    const t = p.solver.globalState.canonicalizeToken(new tokens_1.AllocationSiteToken(kind, p.path.node));
    p.solver.globalState.patching?.registerAllocationSite(t);
    return t;
}
exports.newSpecialObject = newSpecialObject;
function newPackageObject(kind, p) {
    return p.solver.globalState.canonicalizeToken(new tokens_1.PackageObjectToken(p.moduleInfo.packageInfo, kind));
}
...
function addInherits(t, proto, p) {
    const pv = p.op.expVar(proto, p.path);
    if (pv !== undefined)
        p.solver.addInherits(t, pv);
}
exports.addInherits = addInherits;
...
function assignIteratorValuesToProperty(param, t, prop, p) {
    ...
    if ((0, types_1.isExpression)(arg)) {
        ...
        p.op.readIteratorValue(src, dst, arg);
    }
}
...
function assignIteratorMapValuePairs(param, t, keys, values, p) {
    ...
    if ((0, types_1.isExpression)(arg)) {
        ...
        p.op.readIteratorValue(src, dst, arg);
        p.solver.addForAllTokensConstraint(dst, listeners_1.TokenListener.NATIVE_ASSIGN_ITERATOR_MAP_VALUE_PAIRS, p.path.node.arguments[param], (t2) => {
...
function assignBaseArrayArrayValueToArray(t, p) {
    if (p.base instanceof tokens_1.ArrayToken) {
        p.solver.addForAllTokensConstraint(p.solver.varProducer.arrayAllVar(p.base), listeners_1.TokenListener.NATIVE_ASSIGN_BASE_ARRAY_ARRAY_VALUE_TO_ARRAY, { n: p.path.node, t }, (t2) => {
            if (t2 instanceof tokens_1.ArrayToken) {
                const dst = p.solver.varProducer.arrayUnknownVar(t);
...
function returnPrototypeOf(p) {
    ...
    if ((0, types_1.isExpression)(arg) && !(0, asthelpers_1.isParentExpressionStatement)(p.path) && dst !== undefined)
        p.solver.addForAllTokensConstraint(p.solver.varProducer.expVar(arg, p.path), listeners_1.TokenListener.NATIVE_RETURN_PROTOTYPE_OF, p.path.node, (t) => {
            if ((0, constraintvars_1.isObjectPropertyVarObj)(t)) {
                ...
                if (t instanceof tokens_1.ObjectToken)
                    p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.OBJECT_PROTOTYPE), dst);
                else if (t instanceof tokens_1.ArrayToken)
                    p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.ARRAY_PROTOTYPE), dst);
                else if (t instanceof tokens_1.FunctionToken || t instanceof tokens_1.PrototypeToken || t instanceof tokens_1.ClassToken)
                    p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.FUNCTION_PROTOTYPE), dst);
                else if (t instanceof tokens_1.AllocationSiteToken) {
                    if (t.kind === "Promise")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.PROMISE_PROTOTYPE), dst);
                    else if (t.kind === "Date")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.DATE_PROTOTYPE), dst);
                    else if (t.kind === "RegExp")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.REGEXP_PROTOTYPE), dst);
                    else if (t.kind === "Error")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.ERROR_PROTOTYPE), dst);
                    else if (t.kind === "Map")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.MAP_PROTOTYPE), dst);
                    else if (t.kind === "Set")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.SET_PROTOTYPE), dst);
                    else if (t.kind === "WeakMap")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.WEAKMAP_PROTOTYPE), dst);
                    else if (t.kind === "WeakSet")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.WEAKSET_PROTOTYPE), dst);
                    else if (t.kind === "WeakRef")
                        p.solver.addTokenConstraint(p.globalSpecialNatives.get(ecmascript_1.WEAKREF_PROTOTYPE), dst);
...
function setPrototypeOf(p) {
    ...
    if ((0, types_1.isExpression)(obj) && (0, types_1.isExpression)(prototype)) {
        ...
        if (pvar)
            p.solver.addForAllTokensConstraint(p.solver.varProducer.expVar(obj, p.path), listeners_1.TokenListener.NATIVE_SET_PROTOTYPE_OF, p.path.node, (t) => {
                if ((0, constraintvars_1.isObjectPropertyVarObj)(t))
