...
commander_1.program
    ...
    .addHelpText("before", options_1.COPYRIGHT)
...
async function main() {
    ...
    if (options_1.options.dynamic) {
        ...
        if (options_1.options.testGraal) {
        ...
        if (options_1.options.npmTest) {
            ...
            cmd = `${__dirname}${path_1.sep}..${path_1.sep}bin${path_1.sep}node`;
        ...
        const t = (0, child_process_1.spawnSync)(cmd, args, {
            ...
            env: {
                ...
                PATH: `${__dirname}${path_1.sep}..${path_1.sep}bin${path_1.default.delimiter}${process.env.PATH}`,
            },
        });
        if (t.status === null) {
        ...
        if (options_1.options.approxOnly) {
            const p = new processmanager_1.ProcessManager();
            try {
                await p.analyzeFiles(files);
            }
            finally {
                p.saveHintsToFile(options_1.options.approxOnly);
                if (options_1.options.diagnostics)
                    p.printDiagnostics();
                if (options_1.options.diagnosticsJson)
                    p.saveDiagnosticsToFile(options_1.options.diagnosticsJson);
                p.stop();
            }
        }
        else if (options_1.options.typescriptLibraryUsage) {
